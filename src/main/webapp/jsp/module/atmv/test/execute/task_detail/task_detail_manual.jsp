...
<script type="text/javascript">
	...
	function query_data() {
		post_submit("query_data", "task_detail_manual.jsp", "_self");
	}
	function gen_data() {
		post_submit("query_data", get_path() + "/edi/generate_manual", "_self");
	}
</script>
</head>
<%
	...
	String req_select = "row,function,testcase,last_status,concurrent_total,cycle_total,action";
	String req_view = "序号,所属功能,测试用例,状态,并发数,循环数,操作";
...
<body>
 ...
 %>
						...
						<button class="btn btn-xs btn-default btn-me" id="gen_data">立即执行</button>
					</td>
				</tr>
			</table>
		</div>
					...
					for (int i = 0; i < data_total; i++) {
						...
						for (int j = 0; j < selects.length; j++) {
							...
							if (selects[j].trim().equals("last_status")) {
								out.println("<td>");
								out.println("<div>" + ProjectUdc.get_udc_value("tc_status", HandleDatabase.get_string(dtm_02v, 0, "last_status")) + "</div>");
								out.println("</td>");
								continue;
							}
