...
<script type="text/javascript">
	$(document).ready(function() {
		$("#query_data").click(function() {
			post_submit("query_data", "reporter_logdetail-sum.jsp", "_self");
			return false;
		});
	});
</script>
...
<%
	...
	if (sum_type.equals("测试功能")) {
		String req_sql = "SELECT a.fun_code AS static_name,COUNT(1) AS total_num FROM t_funtest_log_detail a WHERE 1=1 AND a.tc_type='case' AND a.operate_ip='"
		+ operate_ip + "' GROUP BY a.fun_code";
	...
	values.put(i, total_num);
		...
		String req_sql = "SELECT a.log_result AS static_name,COUNT(1) AS total_num FROM t_funtest_log_detail a WHERE 1=1 AND a.tc_type='case' AND a.operate_ip='"
		+ operate_ip + "' GROUP BY a.log_result";
		DefaultTableModel dtm_data = ProjectDatabase.query_data(req_sql);
		for (int i = 0; i < dtm_data.getRowCount(); i++) {
