...
<script type="text/javascript">
	...
	function query_data() {
		post_submit("query_data", "td_list.jsp", "_self");
	}
	function add_data() {
		post_submit("query_data", "td_add.jsp", "_self");
	}
	function modify_data() {
		...
		post_submit("query_data", "td_update.jsp", "_self");
	}
	function delete_data() {
		...
		if (is_operate) {
			post_submit("query_data", "td_delete.jsp", "_self");
		}
	}
	function set_data() {
		var selvs = document.getElementsByName("tc_id");
		...
		var is_operate = confirm('是否确认操作？');
		if (is_operate) {
			post_submit("query_data", "td_set.jsp", "_self");
		}
	}
</script>
</head>
<%
	...
	if (cycle_num.length() == 0) {
		cycle_num = "1";
	}
	String req_sql = "select a.* from t_funtest_tc a where tc_type='data'";
	if (fun_code.length() > 0) {
		req_sql = req_sql + " and a.fun_code='" + fun_code + "'";
	}
	if (tc_code.length() > 0) {
		req_sql = req_sql + " and a.tc_code like '%" + tc_code + "%'";
	}
	...
	String req_view = "序号,编号,数据编码,数据路径,数据摘要,执行插件,操作";
	String[] selects = req_select.split(",");
	String[] views = req_view.split(",");

	String menu_name = "测试数据信息";
		...
		<div style="float: left; position: fixed; width: 100%; height: 30px; background: #ffffff; z-index: 999;">
			<table width="100%" align="left">
				<tr valign="middle">
					...
					<td align="right" style="border-bottom: solid 2px #cccccc;">数据编码&nbsp;<input type='text' name="tc_code" value="<%=tc_code%>" size="10" /> 数据摘要&nbsp;<input type='text' name="tc_summary"
						value="<%=tc_summary%>" size="15" /> 状态： <%
					...
					for (int i = 0; i < data_total; i++) {
						...
						for (int j = 0; j < selects.length; j++) {
							...
							if (selects[j].trim().equals("action")) {
								out.println("<td>");
								out.println("<div><a href='" + HandleRequest.get_path() + "/jsp/module/atmv/test/scenario/test_data/td_update.jsp?tc_id=" + tc_idv + "'>修改数据</a></div>");
								...
								out.println("<div><a href='" + HandleRequest.get_path() + "/jsp/module/atmv/test/scenario/test_data/td_gen.jsp?tc_id=" + tc_idv + "'>数据构造</a></div>");
								out.println("</td>");
								continue;
							}
